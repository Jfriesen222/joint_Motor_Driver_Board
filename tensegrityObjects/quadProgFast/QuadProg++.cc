                      ...
                      Vector<double>& x)
{
  ...
  if (G.nrows() != n)
  {
    mexPrintf("The matrix G is not a squared matrix");
  }
  if (CE.nrows() != n)
  {
    mexPrintf("The matrix CE is incompatible (incorrect number of rows)");
  }
  if (ce0.size() != p)
  {
    mexPrintf("The vector ce0 is incompatible (incorrect dimension ");
  }
  if (CI.nrows() != n)
  {
    mexPrintf("The matrix CI is incompatible (incorrect number of rows ");
  }
  if (ci0.size() != m)
  {
    mexPrintf("The vector ci0 is incompatible (incorrect dimension ");
  }
  x.resize(n);
...
#ifdef TRACE_SOLVER
  mexPrintf("Unconstrained solution: %f \r\n",f_value);
  print_vector("x", x);
#endif
  ...
  for (i = 0; i < p; i++)
  {
    ...
    {	  
      ...
      mexPrintf("Constraints are linearly dependent");
      return f_value;
    }
  }
...
#ifdef TRACE_SOLVER
  mexPrintf("Trying with constraint ");
  print_vector("np", np);
#endif
...
l2a:/* Step 2a: determine step direction */
  ...
  t = std::min(t1, t2);
#ifdef TRACE_SOLVER
  mexPrintf("Step sizes: t: %f t1: %f t2: %f \r\n",t,t1,t2);
#endif
...
#ifdef TRACE_SOLVER
    mexPrintf(" in dual space: %f " ,f_value);
    print_vector("x", x);
    print_vector("z", z);
    print_vector("A", A, iq + 1);
#endif
  ...
  u[iq] += t;
#ifdef TRACE_SOLVER
  mexPrintf(" in both spaces: %f",f_value);
  print_vector("x", x);
  print_vector("u", u, iq + 1);
  print_vector("r", r, iq + 1);
  print_vector("A", A, iq + 1);
  ...
  {
#ifdef TRACE_SOLVER
    mexPrintf("Full step has taken %f" , t);
    print_vector("x", x);
#endif
    /* full step has taken */
...
#ifdef TRACE_SOLVER
  mexPrintf("Partial step has taken %f", t );
  print_vector("x", x);
#endif
  /* drop constraint l */
  iai[l] = l;
    ...
    for (j = i; j < n; j++)
    {
      ...
      if (i == j) 
	      ...
	      if (sum <= 0.0)
        {
          // raise error
          print_matrix("A", A);
          mexPrintf("Error in cholesky decomposition, sum: ");
          exit(-1);
        }
	      A[i][i] = sqrt(sum);
	    }

classdef TensegrityStructure < handle
    ...
    methods
        ...
        function staticTensions = getStaticTensions(obj,minForceDensity)
            U = obj.nodePoints([obj.simStruct.topNs obj.simStruct.topNb],:)...
                - obj.nodePoints([obj.simStruct.botNs obj.simStruct.botNb],:);
            if(max(size(obj.C)) >150)
                A= [bsxfun(@times,obj.C',U(:,1)');
                    bsxfun(@times,obj.C',U(:,2)');
                    bsxfun(@times,obj.C',U(:,3)')];
            else
                A= [obj.C' *diag(U(:,1));
                    obj.C' *diag(U(:,2));
                    obj.C' *diag(U(:,3))];
